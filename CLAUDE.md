...
- **`projects` table** (renamed from `customer_projects` in migration 025) holds project-level metadata (`name`, `project_type`, `status`, `zoho_project_id`, `sanity_project_id`, `github_repo`, `dedicated_developers`, `customer_product_id`) keyed by `customer_id`. A customer can have multiple projects. `project_type` is one of: `Content Site | Ecommerce (B2C) | Ecommerce (B2B) | Custom App`. The column `project_name` was renamed to `name` — use `.from("projects")` and `row.name`, never `row.project_name`.
