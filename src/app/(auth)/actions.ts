...
export async function postLoginGate(
  ...
  if (dsErr && dsErr.code !== "PGRST116") {
    ...
    return { redirect: "/auth/login", error: "Something went wrong checking your device. Please try again." };
  }
  ...
  if (!deviceSession || deviceSession.last_verified_at < sevenDaysAgo) {
    ...
    if (otpErr) {
      ...
      return { redirect: "/auth/verify", error: "Could not generate a verification code. Please try resending." };
    }
    ...
    return { redirect: "/auth/verify", warning };
  }
