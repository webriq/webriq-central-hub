...
// PATCH/DELETE a single time_logs entry against an ticket (task 237) — adapted from
...
// PATCH /api/v2/tickets/[ticketId]/time-logs/[timeLogId] — owner-only edit
export async function PATCH(
  req: NextRequest,
  { params }: { params: Promise<{ ticketId: string; timeLogId: string }> }
) {
  const { ticketId, timeLogId } = await params;
  const supabase = await createClient();
  const { data: { user } } = await supabase.auth.getUser();
  if (!user) return NextResponse.json({ error: "Unauthorized" }, { status: 401 });

  const { error: ownError } = await requireOwnRow(supabase, ticketId, timeLogId, user.id);
...
// DELETE /api/v2/tickets/[ticketId]/time-logs/[timeLogId] — owner-only delete
