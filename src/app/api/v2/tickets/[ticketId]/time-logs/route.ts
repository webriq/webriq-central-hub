...
// `issue_id`. `time_logs.issue_id` already existed before this task (task 234's ticket timer
// writes to it); `time_logs_developer_read_all` (migration 094) is role-scoped, not
// `task_id`-scoped, so it already covers ticket-linked rows with zero RLS changes. Writes stay
...
// GET /api/v2/tickets/[ticketId]/time-logs — every entry for the ticket, newest date first
...
// POST /api/v2/tickets/[ticketId]/time-logs — manual entry, assignee-only. Not gated by
// `getTicketEditPermission` (task 237 requirement 6) — logging your own time against an ticket is
...
export async function POST(
  ...
  if (error) {
    console.error("[api/v2/tickets/[id]/time-logs] insert failed:", error.message);
