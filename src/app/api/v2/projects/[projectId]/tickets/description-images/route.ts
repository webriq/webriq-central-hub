...
// Images pasted/dropped into the Ticket Detail rich text description (task 234). Mirrors
// .../tasks/description-images/route.ts exactly, including its role gate — a developer creator
// with full edit rights still can't paste inline images into their own ticket description, same
...
// storage (`storagePath` is keyed by `project.id`, not task/ticket id), not task-specific, so no
...
export async function POST(
  ...
  if (uploadError) {
    console.error("[api/v2/projects/[id]/tickets/description-images] upload failed:", uploadError.message);
