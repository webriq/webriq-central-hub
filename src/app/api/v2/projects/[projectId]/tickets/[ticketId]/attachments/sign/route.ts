...
export async function POST(
  ...
  const storagePath = `issues/${ticket.id}/${Date.now()}_${safeFilename}`;

  try {
    const signed = await createAttachmentUploadUrl(supabase, storagePath);
    return NextResponse.json(signed);
  } catch (e) {
    console.error("[api/v2/projects/[id]/tickets/[ticketId]/attachments/sign] failed:", e instanceof Error ? e.message : e);
