...
// Task 235 — Attachments tab for Ticket Detail. Reuses the existing polymorphic `attachments`
...
// `getTicketEditPermission` (task 234) directly.
//
...
export async function GET(
  ...
  // Task 257, Requirement F — merge in attachments uploaded on this ticket's comments
...
export async function POST(
  ...
  // ticket's folder — reject anything else outright.
  if (!storagePath.startsWith(`issues/${ticket.id}/`)) {
  ...
  if (error) {
    await supabase.storage.from("project-assets").remove([storagePath]);
    console.error("[api/v2/projects/[id]/tickets/[ticketId]/attachments] insert failed:", error.message);
