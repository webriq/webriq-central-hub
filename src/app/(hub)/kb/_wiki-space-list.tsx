...
import { PageRow, buildTree, type EditingPages } from "./_wiki-tree-panel-rows";
...
function SortableSpaceRow({
  ...
  return (
    <div ref={setNodeRef} style={style} className={cn("mb-0.5", isDragging && "z-10 opacity-70")}>
      ...
      >
        {reorderDisabled ? (
          <span className="h-8 w-5 shrink-0" />
        ) : (
          <button
            type="button"
            {...attributes}
...
export function WikiSpaceList({
  ...
  // Task 401 — Spaces filter. Hidden spaces are skipped at render time rather than dropped from
  // `order`, and drag is disabled meanwhile, so a reorder can never persist a truncated order.
  visibleSpaces?: Set<WikiProduct>;
  editingPages?: EditingPages;
}) {
  const reorderDisabled = visibleSpaces !== undefined;
  const sensors = useSensors(useSensor(PointerSensor, { activationConstraint: { distance: 4 } }));
  // dnd-kit derives its a11y ids (`DndDescribedBy-N`) from a module-level counter that differs
  // between the server render and hydration; a React `useId` is identical on both sides.
  const dndId = useId();
    ...
    <DndContext id={dndId} sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
      <SortableContext items={order} strategy={verticalListSortingStrategy}>
        {order.map((name) => {
          ...
          if (!space || (visibleSpaces && !visibleSpaces.has(name))) return null;
          const isOpen = expanded.has(name);
          const tree = buildTree(pages, name);
          return (
            <SortableSpaceRow key={name} product={space} isOpen={isOpen} onToggle={() => onToggleSpace(name)} reorderDisabled={reorderDisabled}>
              <div className="pl-[26px] border-l border-[#E2E7F2] ml-4">
                {tree.length === 0 ? (
                  <p className="px-2.5 py-1.5 text-[11.5px] text-[#94A3B8]">No pages yet</p>
                ) : (
                  tree.map((node) => (
                    <PageRow key={node.id} node={node} depth={0} selectedPageId={selectedPageId} onSelect={(id) => onSelectPage(id, name)} editingPages={editingPages} />
                  ))
                )}
              </div>
            </SortableSpaceRow>
