...
// [projectId]/tickets/[ticketId]/comments/[commentId]/attachments/route.ts).
...
// (`issue_comments.source_meta.attachments`, populated by zoho-import/ticket-comments/route.ts)
...
export function TicketComments({
  ...
  useEffect(() => {
    ...
    const commentsChannel = supabase
      ...
      .on(
        "postgres_changes",
        { event: "*", schema: "public", table: "issue_comments", filter: `issue_id=eq.${ticketId}` },
        (payload) => {
    ...
    const attachmentsChannel = supabase
      .channel(`ticket_comment_attachments_${ticketId}`)
      .on(
        "postgres_changes",
  ...
  async function postComment() {
    ...
    const res = await fetch(`/api/v2/projects/${projectId}/tickets/${ticketId}/comments`, {
      method: "POST",
    ...
    if (res.ok) {
      ...
      if (attachmentFiles.length > 0) {
        const results = await Promise.allSettled(attachmentFiles.map((file) => {
          ...
          fd.append("file", file);
          return fetch(`/api/v2/projects/${projectId}/tickets/${ticketId}/comments/${created.id}/attachments`, { method: "POST", body: fd })
  ...
  async function deleteComment(commentId: string) {
    ...
    const res = await fetch(`/api/v2/projects/${projectId}/tickets/${ticketId}/comments/${commentId}`, { method: "DELETE" });
    setDeletingId(null);
    if (res.ok) {
    ...
    <div className="flex flex-col gap-4">
              ...
              <div className="flex-1 min-w-0">
                ...
                {c.attachments.length > 0 && (
                  <CommentAttachmentGrid
                    items={c.attachments}
                    renderItem={(file) => {
                      const fetchUrl = `/api/v2/projects/${projectId}/tickets/${ticketId}/comments/${c.id}/attachments/${file.id}/file-url`;
      ...
      {viewing && (
        <TaskAttachmentViewerModal
          ...
          fetchUrl={`/api/v2/projects/${projectId}/tickets/${ticketId}/comments/${viewing.commentId}/attachments/${viewing.attachment.id}/file-url`}
