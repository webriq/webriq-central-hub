...
import { StatusBadge, SeverityBadge, decodeHtmlEntities, type TaskStatus } from "@/app/(hub)/projects-old/_pm-shared";

// Sibling task/ticket navigation list (task 257, Requirement H) — Zoho's reference shows a
// persistent left-hand list of other tickets for fast navigation without returning to the list
// view. Scoped to same-project, assigned-to-current-user (with a same-project open-tickets
...
export type QuickAccessTicket = { id: string; display_id: string; title: string; status: string; severity: string | null };
...
export function TicketQuickAccessPanel({
  tasks,
  tickets,
  projectId,
}: {
  tasks: QuickAccessTask[];
  tickets: QuickAccessTicket[];
  projectId: string;
}) {
    ...
    <ul className="flex flex-col gap-1 -m-1.5">
      {tickets.map((i) => (
        <li key={`ticket-${i.id}`}>
          <button
            type="button"
            onClick={() => router.push(`/projects/legacy/${projectId}/tickets/${i.display_id}`)}
      ...
      {tasks.map((t) => (
        <li key={`task-${t.id}`}>
          <button
            type="button"
            onClick={() => router.push(`/projects/legacy/${projectId}/tasks/${t.display_id}`)}
            className="w-full flex items-center gap-2 px-1.5 py-1.5 rounded-[8px] text-left cursor-pointer transition-colors hover:bg-[#F4F6FB]"
          >
            <TypeChip label="TASK" />
            <span className="flex-1 min-w-0 truncate text-[12px] text-[#3A4565]">
