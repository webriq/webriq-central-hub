...
// Time Logs tab for Ticket Detail (task 237) — adapted from
...
// viewer with access to the ticket sees every entry logged against it (migration 094's
// developer-read-all policy, role-scoped not task_id-scoped, already covers ticket-linked rows),
...
export function TicketTimeLogs({ ticketId, refreshKey }: { ticketId: string; refreshKey?: number }) {
  ...
  const apiBasePath = `/api/v2/tickets/${ticketId}/time-logs`;
  ...
  useEffect(() => {
    ...
    // ticket timer, wired through `_issue-detail.tsx`'s `handleHoursLogged`).
