...
export default function ProjectsIndex({
  ...
  function buildUrl(overrides: Record<string, string | number | null>) {
    ...
    return `${V2_ROUTES.PROJECTS_LEGACY}?${p.toString()}`;
  }
      ...
      <div className={cn("sticky top-0 z-20 bg-[#F4F6FB] transition-shadow duration-150", scrolled && "shadow-[0_1px_0_0_rgba(7,17,51,0.06)]")}>
        <div className="max-w-[1400px] mx-auto px-8 pt-6 pb-4">
          ...
          <div className="flex items-center justify-between gap-4 mb-4">
            <div>
              <p className="text-[13px] text-[#5F6A88]">
                {total} project{total === 1 ? "" : "s"} · Original StackShift & Discrete Development — predate onboarding and the 120-day timeline
            ...
            {canCreateProject && (
              <Tooltip>
                <TooltipTrigger render={
                  <button
                    aria-label="Where to create a new project"
                    className="inline-flex shrink-0 cursor-pointer items-center justify-center rounded-full p-1.5 text-[#5F6A88] transition-colors hover:text-[#0B1533]"
                  >
                    <Info size={16} />
                  </button>
                } />
                <TooltipContent side="left" className="max-w-56 items-start text-left">
                  {/* Base TooltipContent is `inline-flex` — without a single wrapping element here,
                      the flex row splits the text node and <Link> into separate items instead of
                      letting them wrap together as one line of text. */}
                  <span>
                    New projects are now created in{" "}
                    <Link href={V2_ROUTES.PROJECTS_V2_NEW} className="underline hover:no-underline">
                      V2 Projects
                    </Link>.
                  </span>
                </TooltipContent>
          ...
          <div className="flex items-center gap-3 flex-wrap">
            ...
            {isFiltered && (
              <button
                onClick={() => { setSearchInput(""); navigate(V2_ROUTES.PROJECTS_LEGACY); }}
