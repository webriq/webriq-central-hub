...
// Rich-text comment composer for Ticket Detail (task 236) — copy-adapted from
// ../../tasks/[taskId]/_comment-editor.tsx (kept separate, not shared/generalized, since the
// task-side component and its `task_comments`/RLS are explicitly out of scope for this task).
// Same lifecycle as the task version: tracks content live via onUpdate, resets via a
// parent-driven `key` remount (see TicketComments).
export function TicketCommentEditor({
  ...
  async function uploadAndInsertImage(file: File) {
    ...
    fd.append("file", file);
    const res = await fetch(`/api/v2/projects/${projectId}/tickets/${ticketId}/comments/description-images`, {
      method: "POST",
      body: fd,
    });
    setUploading(false);
