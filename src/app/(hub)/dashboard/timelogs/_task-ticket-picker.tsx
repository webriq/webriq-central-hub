...
// Task/Tickets field for the Add/Edit Time Log modal, redesigned as a search-only trigger (no
// chevron) opening a Tasks/Tickets-tabbed, scroll-revealed panel, with a switch to a free-text
...
//
// `GET /api/v2/projects/[projectId]/tasks` and `.../tickets` both already return their *complete*
...
export type TaskTicketValue =
  | { kind: "task"; id: string; label: string; displayId: string | null }
  | { kind: "ticket"; id: string; label: string; displayId: string | null }
...
function selectedKind(value: TaskTicketValue | null): "task" | "ticket" | null {
  if (!value || value.kind === "general") return null;
  return value.kind;
}

function selectedId(value: TaskTicketValue | null): string | null {
  if (!value || value.kind === "general") return null;
  return value.id;
}
...
export function TaskTicketPicker({
  ...
  value: TaskTicketValue | null;
  onChange: (v: TaskTicketValue | null) => void;
  ...
  const [tickets, setTickets] = useState<PickerItem[]>([]);
    ...
    startFetchTransition(async () => {
      ...
      try {
        const [taskData, ticketData]: [
          ...
          fetch(`/api/v2/projects/${projectId}/tickets`, { signal: ctrl.signal }).then((r) => (r.ok ? r.json() : [])),
        ]);
        ...
        setTickets(sortNewestFirst(ticketData.map((i) => ({ id: i.id, title: i.title, created_at: i.created_at, displayId: i.display_id }))));
      } catch {
        setTasks([]);
        setTickets([]);
      } finally {
  ...
  const activeList = tab === "tasks" ? tasks : tickets;
  ...
  function pick(kind: "task" | "ticket", item: PickerItem) {
        ...
        >
          ...
          <div className="max-h-[220px] overflow-y-auto p-1" onScroll={handleScroll}>
            {loading ? (
              ...
              visible.map((item) => {
                const kind = tab === "tasks" ? "task" : "ticket";
