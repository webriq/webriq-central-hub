...
function AssessmentResult({ record }: { record: RequirementsAssessmentRow }) {
  const subtasks = (record.subtasks as unknown as Array<{ title: string; status: string; notes?: string }>) ?? [];
...
function PlanResult({
  ...
  const steps = (plan.steps as unknown as Array<{ order: number; title: string; description: string; estimated_hours?: number }>) ?? [];
  const affectedFiles = (plan.affected_files as unknown as string[]) ?? [];
  const riskFlags = (plan.risk_flags as unknown as string[]) ?? [];
        ...
        <div className="mb-3 px-2.5 py-2 bg-red-500/4 rounded-[7px] border border-red-500/12">
          ...
          <ul className="list-disc list-inside">
            {riskFlags.map((flag, i) => (
              <li key={i} className="text-[11px] text-red-600 mt-0.5">{flag}</li>
            ))}
          </ul>
        </div>
      )}
...
type StageFilter = "all" | "assess" | "plan";
...
export default function OrchestrationContent() {
  ...
  useEffect(() => {
    ...
    async function load() {
      const [tasksResult, assessmentsResult, plansResult, zohoProjectsResult, executionsResult, pausedResult, replyDraftsResult, customerNamesResult] = await Promise.all([
        supabase
      ...
      for (const c of (customerNamesResult.data ?? []) as Array<{ customer_id: string; company_name: string }>) {
        namesMap[c.customer_id] = c.company_name;
      }
  ...
  return (
    <div className="max-w-4xl mx-auto px-8 py-6.5">

      {Object.keys(customerPaused).length > 0 && (
        <div className="mb-4 px-4 py-3 bg-yellow-50 border border-yellow-200 rounded-xl text-yellow-800 text-[13px] flex items-start gap-2">
          <AlertTriangle size={16} className="shrink-0 mt-0.5" />
          <span>
            Automation paused for{" "}
      ...
      {/* ── Page header ─────────────────────────────────────────────── */}
      <div className="mb-5">
        <div className="text-[22px] font-bold text-slate-900 tracking-[-0.02em]">
          Orchestration
        </div>
        <div className="text-[11px] text-slate-400 mt-0.5">
          Assess → Plan → Execute → Reply · LLM-eligible tasks
        </div>
      </div>
      ...
      <div className="flex gap-1.5 flex-wrap mb-6">
        {STAGE_TABS.map(s => {
          const count = s.key === "assess" ? assessmentTasks.length : s.key === "plan" ? planTasks.length : assessmentTasks.length + planTasks.length;
          return (
            <button
              key={s.key}
              onClick={() => setStageFilter(s.key)}
              className={cn(
                "text-[12px] font-medium px-3 py-1.5 rounded-full border transition-colors cursor-pointer",
                stageFilter === s.key
                  ? "bg-[#3358f4] text-white border-[#3358f4]"
      ...
      {(stageFilter === "all" || stageFilter === "assess") && (
      <div className="mb-6">
        <div className="text-[13px] font-semibold text-slate-700 tracking-[-0.01em] mb-3">
      ...
      {!loading && (stageFilter === "all" || stageFilter === "assess") && assessmentTasks.length === 0 && (
        <div className="text-center p-6 bg-white rounded-xl border border-black/7 text-slate-400 text-[13px] mb-6">
      ...
      {!loading && (stageFilter === "all" || stageFilter === "assess") && (
        <div className="flex flex-col gap-2.5 mb-6">
          {assessmentTasks.map(task => (
            <TaskRow
              ...
              customerName={customerNames[task.customer_id] ?? task.customer_id}
            />
          ))}
        </div>
      )}
      ...
      {!loading && (stageFilter === "all" || stageFilter === "plan") && (
        <div className={stageFilter === "all" ? "mt-4" : ""}>
          <div className="mb-3">
            <div className="text-[13px] font-semibold text-slate-700 tracking-[-0.01em]">
              Plan Generation
            </div>
          </div>
      ...
      <div className="mt-12 text-center text-[10px] text-slate-300">
        Execute · Reply stages coming Sprint 5+
      </div>
    </div>
  );
}
