...
import { type Ticket, SEVERITY_STYLE, normalizeSeverity, decodeHtmlEntities } from "../_pm-shared";
...
export default function IssueCalendarView({
  issues,
  onOpen,
}: {
  issues: Ticket[];
  onOpen: (issue: Ticket) => void;
}) {
  ...
  const unscheduled = issues.filter((i) => !i.due_date).length;
  ...
  return (
    <div className="h-full overflow-y-auto px-8 py-5">
      <div className="flex items-center justify-between mb-4">
        ...
        {unscheduled > 0 && (
          <span className="text-[12px] text-[#5F6A88]">{unscheduled} issue{unscheduled === 1 ? "" : "s"} without a due date</span>
        )}
      </div>
                  ...
                  {dayIssues.slice(0, 3).map((issue) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <span className="text-[11px] text-[#3A4565] truncate">{decodeHtmlEntities(issue.title)}</span>
                      </button>
                    );
                  })}
                  {dayIssues.length > 3 && (
                    <span className="text-[10px] text-[#5F6A88] pl-1">+{dayIssues.length - 3} more</span>
                  )}
                </div>
              </div>
            );
