  ...
  return (
    <div className="flex flex-col h-full min-h-0">
      ...
      <div className="px-8 pt-6 pb-0 bg-white shrink-0">
        <button
          onClick={() => router.push(V2_ROUTES.PROJECTS_OLD)}
        ...
        <div className="flex items-center mt-4">
          <div className="flex items-center gap-1 bg-[#F4F6FB] rounded-full p-1">
            {PRIMARY_TABS.map((tab) => (
              <button
                key={tab.id}
                onClick={() => router.push(`/projects-old/${project.project_id}/${tab.id}`)}
                className={cn(
            ...
            <div className="flex-1 min-h-0 overflow-hidden">
              {view === "board" && (
                <BoardView
                  ...
                  onOpen={(task) => router.push(`/projects-old/${project.project_id}/tasks/${task.display_id}`)}
            ...
            <div className="flex-1 min-h-0 overflow-hidden">
              {issueView === "board" && (
                <IssueBoardView
                  ...
                  onOpen={(issue) => router.push(`/projects-old/${project.project_id}/issues/${issue.display_id}`)}
                />
              )}
              {issueView === "list" && (
                <IssueListView
