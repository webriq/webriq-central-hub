...
import { IssueAttachmentsCommentsPanel } from "./_ticket-attachments-comments-panel";
import { IssueQuickAccessPanel, type QuickAccessTask, type QuickAccessIssue } from "./_ticket-quick-access-panel";
...
export default function IssueDetailClient({
  ...
  const perm = getTicketEditPermission(currentUserRole, currentUserId, issue);
  ...
  const [severity, setSeverity] = useState<TicketSeverity>(normalizeSeverity(issue.severity));
  ...
  const goToIssues = useCallback(() => {
    if (project.project_id) router.push(`/projects-old/${project.project_id}/issues`);
