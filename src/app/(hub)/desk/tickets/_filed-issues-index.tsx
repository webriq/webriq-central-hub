...
import { ChevronLeft, ChevronRight, ChevronsLeft, ChevronsRight, Bug, Search, X } from "lucide-react";
...
export default function FiledIssuesIndex({
  ...
  async function updateIssue(id: string, patch: { status?: string; assignees?: string[] }): Promise<boolean> {
    ...
    setIssues((prev) => prev.map((i) => (i.id === id
      ? { ...i, ...(patch.status ? { status: patch.status } : {}), ...(patch.assignees ? { assignees: patch.assignees, assigneeId: patch.assignees[0] ?? null } : {}) }
      : i)));
            ...
            <div className="flex flex-col items-center justify-center py-20 gap-3 rounded-[14px] border border-[#E2E7F2] bg-white">
              ...
              <div className="text-center">
                ...
                <p className="text-[13px] text-[#5F6A88] mt-1">
                  Issues filed from an Inbox thread message (&ldquo;File an Issue&rdquo;) will appear here.
                </p>
              </div>
            </div>
          )
        ) : (
          <FiledIssuesTable issues={issues} allMembers={members} onUpdate={updateIssue} />
        )}
      </div>
    </div>
  );
