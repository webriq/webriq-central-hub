...
export default async function DeskFiledIssuesPage({
  ...
  if (statusSelected.length === 0) {
    ...
    issuesQuery = issuesQuery.eq("id", "00000000-0000-0000-0000-000000000000");
  } else if (statusSelected.length < ALL_STATUS_VALUES.length) {
    issuesQuery = issuesQuery.in("status", statusSelected);
  }
  if (searchQ) {
    const esc = searchQ.replace(/[%,()]/g, "");
    issuesQuery = issuesQuery.or(`title.ilike.%${esc}%,display_id.ilike.%${esc}%`);
  }

  const issuesRes = await issuesQuery.range(from, to);
  ...
  return <FiledIssuesIndex issues={issues} paginationMeta={paginationMeta} />;
}
