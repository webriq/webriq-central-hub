...
// Task 333 — "File a Ticket" from a customer-authored Inbox thread message. This is the
// project-picker gate that fronts the Projects New Ticket modal: pick a project (searchable),
// then it loads that project's data bundle and hands off to the real modal with Title seeded
// from the ticket Subject and Description seeded from the message body. The Projects modal is
// reused verbatim (only the optional seed props were added there).
//
// Task 364 dropped the "Create Task" option entirely (this component used to front both New
// Task and New Issue) and renamed "Issue" to "Ticket" throughout — this is now a single-purpose
// ticket-filing gate, reached directly via a button rather than a kebab menu.
  ...
  if (bundle) {
    return (
      <CreateTicketModal
        ...
        // Task 338 made the New Ticket modal's Description a Tiptap RTE (same as New Task), so
    ...
    >
      ...
      >
        <div className="flex items-center justify-between px-5 py-4 border-b border-[#EDF0F7]">
          <h2 className="text-[15px] font-semibold text-[#0B1533]">File a Ticket from message</h2>
          <button
            onClick={onClose}
