...
// Ticket ID/Owner/Responded/Due Date moved to the Tickets tab (still task 363's follow-up) — this
// list only needs what it still displays, plus the reverse "Linked Ticket" lookup. "Status" moved
// back here (task 370) as an editable column — it's the origin ticket's own status, so this row
// is the natural place to change it, not a read-only mirror on the Tickets tab.
...
export default function InboxIndex({
  ...
  // Re-sync local state when the server hands us a new page/filter result — same
  // compare-during-render pattern `_filed-issues-index.tsx` uses (not an effect, so it doesn't
  // trip `react-hooks/set-state-in-effect`'s cascading-render warning).
  ...
  function buildUrl(overrides: Record<string, string | number | null>) {
    ...
    return `${V2_ROUTES.DESK_INBOX}?${p.toString()}`;
  }
      ...
      <div className={cn("sticky top-0 z-20 bg-[#F4F6FB] transition-shadow duration-150", scrolled && "shadow-[0_1px_0_0_rgba(7,17,51,0.08)]")}>
        <div className="max-w-[1400px] mx-auto px-8 pt-6 pb-4">
          ...
          <div className="flex items-center justify-between gap-4 mb-4">
            <div>
              <h1 className="font-heading text-[22px] font-bold tracking-[-0.02em] text-[#0B1533]">Inbox</h1>
        ...
        {tickets.length === 0 ? (
          isFiltered ? (
            <div className="flex flex-col items-center justify-center py-20 gap-3 rounded-[14px] border border-[#E2E7F2] bg-white">
              ...
              <button
                onClick={() => { setSearchInput(""); router.push(`${V2_ROUTES.DESK_INBOX}?status=all`); }}
