...
import { spaceGrotesk, inter } from "../_fonts";
...
// this is the condensed subset this page's compact chips actually need.
const TYPE_CHIP_COLORS: Record<Classification, { bg: string; text: string; border: string }> = {
  "StackShift I": { bg: "bg-[#EFF6FF]", text: "text-[#2563EB]", border: "border-[#2563EB]" },
  "StackShift II": { bg: "bg-[#EFF6FF]", text: "text-[#2563EB]", border: "border-[#2563EB]" },
  "StackShift Access": { bg: "bg-[#F5F3FF]", text: "text-[#7C3AED]", border: "border-[#7C3AED]" },
  "StackShift Access Plus": { bg: "bg-[#F5F3FF]", text: "text-[#7C3AED]", border: "border-[#7C3AED]" },
  PipelineForge: { bg: "bg-[#F0FDFA]", text: "text-[#0D9488]", border: "border-[#0D9488]" },
  "Discrete Development": { bg: "bg-[#FFF7ED]", text: "text-[#F97316]", border: "border-[#F97316]" },
};
...
function TypeMultiSelect({ value, onChange }: { value: Classification[]; onChange: (v: Classification[]) => void }) {
  ...
  return (
    <>
      ...
      <div
        ...
        className={cn(
          "flex min-h-[34px] w-[190px] cursor-pointer flex-wrap items-center gap-1 rounded-[7px] border-[1.5px] bg-white px-2 py-1 text-left outline-none transition-colors",
          open ? "border-[#2563EB]" : "border-[#E2E8F0] hover:border-[#CBD5E1]"
        )}
      >
        {value.length === 0 ? (
          <span className="inline-flex items-center gap-1 text-[11.5px] text-[#94A3B8]">
          ...
          value.map((c) => {
            ...
            return (
              <span
                key={c}
                className={cn("inline-flex items-center gap-1 rounded-md border py-0.5 pl-1.5 pr-1 text-[10px] font-medium", meta.bg, meta.text, meta.border)}
              >
                {c}
                <button
                  type="button"
      ...
      {open &&
        ...
        createPortal(
          <div
            ...
            className="z-50 overflow-hidden rounded-lg border border-[#E2E8F0] bg-white shadow-lg"
          >
            <div className="border-b border-[#F1F5F9] p-2">
              <div className="relative">
                <Search size={12} className="pointer-events-none absolute left-2 top-1/2 -translate-y-1/2 text-[#94A3B8]" />
                <input
                  autoFocus
                  value={query}
                  onChange={(e) => setQuery(e.target.value)}
                  placeholder="Search types…"
                  className="w-full rounded-md border border-[#E2E8F0] py-1 pl-6 pr-2 text-[12px] text-[#0F172A] outline-none placeholder:text-[#94A3B8] focus:border-[#2563EB]"
                />
              </div>
            </div>
            <div className="max-h-[180px] overflow-y-auto p-1">
              {filtered.length === 0 ? (
                <div className="px-2 py-2 text-[11.5px] text-[#94A3B8]">No matches</div>
              ) : (
                filtered.map((c) => {
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        "flex w-full cursor-pointer items-center justify-between gap-2 rounded-md px-2 py-1.5 text-left text-[12px] transition-colors hover:bg-[#F8FAFC]",
                        selected && "bg-[#EFF6FF]"
                      )}
                    >
                      <span className={selected ? "font-medium text-[#0F172A]" : "text-[#475569]"}>{c}</span>
...
export default function ImportProjectWizard() {
  ...
  return (
    <div className={cn(inter.className, "flex min-h-full flex-col items-center bg-[#F8FAFC] px-6 py-10")}>
      ...
      <div className="w-full max-w-[1200px] rounded-2xl border border-[#E2E8F0] bg-white px-10 py-9 shadow-[0_4px_24px_rgba(15,23,42,0.07)]">
        {result ? (
          <motion.div
          ...
          >
            ...
            {result.errors.length > 0 && (
              <div className="mb-6 max-h-[280px] overflow-y-auto rounded-[10px] border border-[#E2E8F0] text-left">
                {result.errors.map((e, i) => (
                  <div
                    key={i}
                    className={cn("px-4 py-2.5 text-[12.5px] text-[#475569]", i > 0 && "border-t border-[#F1F5F9]")}
                  >
                  ...
                  <div>
                    <div className="mb-7">
                      ...
                      <p className="text-[13px] text-[#64748B]">
                        ...
                        <span className="font-medium text-[#0F172A]">Customer, Project Type, Primary Contact, Kickoff Date, Current Phase</span>.
                      </p>
                    </div>
                    ...
                    >
                      <div className="flex h-12 w-12 items-center justify-center rounded-[12px] bg-[#2563EB]/10 text-[#2563EB]">
                        <Upload size={22} />
                      </div>
                      <div>
                        <div className="text-sm font-semibold text-[#0F172A]">Click to upload or drag and drop</div>
                        <div className="mt-0.5 text-xs text-[#94A3B8]">.csv, .xlsx, or .xls</div>
                      </div>
                    </div>

                    {parseError && (
                      <p className="mt-3 flex items-center gap-1.5 text-xs text-[#DC2626]">
                ...
                {step === 2 && (
                  <div>
                    <div className="mb-5 flex items-start justify-between gap-4">
                      ...
                      {attentionCount > 0 ? (
                        <span className="inline-flex shrink-0 items-center gap-1.5 rounded-full border border-[#FED7AA] bg-[#FFF7ED] px-3 py-1.5 text-[11.5px] font-medium text-[#C2410C]">
                          <AlertCircle size={12} /> {attentionCount} row{attentionCount === 1 ? "" : "s"} need attention
                        </span>
                      ) : (
                        <span className="inline-flex shrink-0 items-center gap-1.5 rounded-full border border-[#BBF7D0] bg-[#F0FDF4] px-3 py-1.5 text-[11.5px] font-medium text-[#16A34A]">
                    ...
                    {rows.length === 0 ? (
                      <div className="rounded-xl border border-[#E2E8F0] bg-[#F8FAFC] px-6 py-12 text-center text-sm text-[#94A3B8]">
                        ...
                        <table className="w-full border-collapse text-[12.5px]">
                          ...
                          <tbody>
                            {rows.map((row) => {
                              ...
                              return (
                                <tr key={row.id} className={cn("border-t border-[#F1F5F9]", attention && "bg-[#FFF7ED]/40")}>
                                  <td className="px-3 py-2.5 align-top">
                                    <input
                                      ...
                                      className={cn(
                                        "w-[160px] rounded-[7px] border-[1.5px] bg-white px-2.5 py-1.5 text-[12.5px] text-[#0F172A] outline-none transition-colors",
                                        !row.account.trim() ? "border-[#FCA5A5]" : "border-[#E2E8F0] focus:border-[#2563EB]"
                                      )}
                                    />
                                  </td>
                                  <td className="px-3 py-2.5 align-top">
                                    <TypeMultiSelect value={row.classifications} onChange={(v) => updateRow(row.id, { classifications: v })} />
                                    {row.classifications.length === 0 && (
                                      <div className="mt-1 text-[10.5px] text-[#DC2626]">Select at least one</div>
                                    )}
                                  </td>
                                  <td className="px-3 py-2.5 align-top">
                                    <input
                                      ...
                                      className="w-[140px] rounded-[7px] border-[1.5px] border-[#E2E8F0] bg-white px-2.5 py-1.5 text-[12.5px] text-[#0F172A] outline-none transition-colors focus:border-[#2563EB]"
                                    />
                                  ...
                                  <td className="px-3 py-2.5 align-top">
                                    <input
                                      ...
                                      className="w-[150px] cursor-pointer rounded-[7px] border-[1.5px] border-[#E2E8F0] bg-white px-2.5 py-1.5 text-[12.5px] text-[#0F172A] outline-none transition-colors focus:border-[#2563EB]"
                                    />
                                  </td>
                                  <td className="px-3 py-2.5 align-top">
                                    <select
                                      ...
                                      className="w-[170px] cursor-pointer rounded-[7px] border-[1.5px] border-[#E2E8F0] bg-white px-2.5 py-1.5 text-[12.5px] text-[#0F172A] outline-none transition-colors focus:border-[#2563EB]"
                                    >
                                    ...
                                    {isOverdue(row.kickoffDate, row.currentPhase) && (
                                      <div className="mt-1 text-[10.5px] text-[#DC2626]">
                                  ...
                                  <td className="px-2 py-2.5 align-top">
                                    <button
                                      ...
                                      className="cursor-pointer rounded-md p-1.5 text-[#94A3B8] transition-colors hover:bg-[#FEF2F2] hover:text-[#DC2626]"
                                    >
                                      <Trash2 size={14} />
                                    </button>
                                  </td>
            ...
            {step === 2 && rows.length > 0 && (
              <div className="mt-7 flex items-center justify-between">
                <button
                  ...
                  onClick={goBack}
                  disabled={submitting}
                  className="flex cursor-pointer items-center gap-1.5 rounded-[9px] border-[1.5px] border-[#E2E8F0] bg-transparent px-4 py-2.5 text-[13px] font-medium text-[#475569] transition-colors hover:border-[#CBD5E1] hover:bg-[#F8FAFC] disabled:cursor-not-allowed disabled:opacity-60"
                >
